...
import pyb
import math
import star_finder, astro_sensor
...
for i in stars:
    ...
    for j in profile:
        profile_text += "%u," % j
    text = "%u-[%s]" % (i.star_rating(), profile_text)
    if cx_i >= img.width() // 2:
        text_x = cx_i - r - bar_len - (len(text) * 9) - 2
    else:
        text_x = cx_i + r + bar_len + 2
    if cy_i < img.height() // 2:
        text_y = cy_i + r + bar_len + 5
    else:
        text_y = cy_i - r - bar_len - 5
    img.draw_string(text_x, text_y, text, 255)

fname = "test_%u.jpg" % (pyb.rng() % 1000)
print("saving: " + fname)
